...
void SSD1331::oled_init(){
	...
	//获取pin口对应的数据寄存器以及pin口对应的寄存器位置
#if !defined (__arm__) && !defined (__SAM3X8E__)
    csport      = portOutputRegister(digitalPinToPort(_cs));
    cspinmask   = digitalPinToBitMask(_cs);
    rsport      = portOutputRegister(digitalPinToPort(_rs));
    rspinmask   = digitalPinToBitMask(_rs);
	sclkport    = portOutputRegister(digitalPinToPort(_sclk));
	sclkpinmask = digitalPinToBitMask(_sclk);
	sdinport    = portOutputRegister(digitalPinToPort(_sdin));
	sdinpinmask = digitalPinToBitMask(_sdin);
	rstport     = portOutputRegister(digitalPinToPort(_rst));
	rstpinmask  = digitalPinToBitMask(_rst);
#endif
